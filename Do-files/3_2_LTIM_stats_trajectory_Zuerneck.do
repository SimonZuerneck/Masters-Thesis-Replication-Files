...
foreach v of local outcome {
	...
	*Create a trend plot
	capture graph drop `dataset'_trends_`v'_CI
	twoway ///	
		...
		lpattern(dash)), ///
		legend(order(3 "EU Inflow Estimates" 4 "Non-EU Inflow Estimates") ///
		...
		xlabel(, grid) ///
		xtitle("") ytitle("Inflow Estimates in Thousands") ///
		...
		name(`dataset'_trends_`v'_CI)
					
	*Store the graph 
	local `dataset'_trends_CI "``dataset'_trends_CI' `dataset'_trends_`v'_CI"
		
	restore
}
di "Stored graphs: ``dataset'_trends_CI'"

*Combine graphs and save them as PDF
cd "$master_thesis/Graphs"
grc1leg2 ``dataset'_trends_CI', leg(LTIM_trends_inflow_2_CI) iscale(*.7) ///
graphregion(color(white)) graphregion(margin(zero)) 

*Save the graph 
cd "$master_thesis/Graphs"
graph export "`dataset'_trends_CI_combined.pdf", replace as (pdf)
********************************************************************************
